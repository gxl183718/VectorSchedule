 ...
 * @date 2019-9-5 10:04
 */
public class AccessMediaData implements Runnable {
 ...
 */
    ...
    public AccessMediaData(ClientAPI clientAPI) {
        this.clientAPI = clientAPI;
    }
    public AccessMediaData() {
    }
        ...
        while (start) {
            ...
            try {
                ...
                if (null != ma_picture_files && !ma_picture_files.isEmpty()) {
                    byte[] content = OKClientUtils.doGet(TestConf.loongAddr + ma_picture_files.get(0));
                    md5 = getMd5(content);
                    key = KeyFactory.getInstance(KeyFactory.KeyType.ImageTEST, md5, time);
                    clientAPI.put(key, content);
                    m_type = 3;
                }else if (ma_audio_files !=null || ma_video_files != null){
                    continue;
                }
                System.out.println(".... puti = " + time);
